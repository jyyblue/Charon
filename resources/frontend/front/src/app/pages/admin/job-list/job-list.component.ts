...
export class JobListComponent implements OnInit {
  ...
  get rf(): any {
    return this.resolveForm.controls;
  }
  ...
  onSelectTask(taskid, check) {
    let index = this.taskData.findIndex(item => {
    ...
    if (check) {
      ...
      if (this.selectedTask.includes(this.taskData[index])) {
        ...
        this.selectedTask.splice(
          this.selectedTask.indexOf(this.taskData[index]),
          1
        );
        this.generatePaymentData();
      }
  ...
  generatePaymentData() {
    ...
    this.selectedTask.forEach(task => {
      ...
      let existIndex = this.paymentData.findIndex(pdata => {
        return pdata.driver_id == task.driver_id;
      });
      if (existIndex == -1) {
  ...
  async onSubComponentChange(event: ComponentChangedEvent) {
    ...
    if (action == "edit") {
      ...
      let taskIdx = this.taskData.findIndex(item => {
        return item["id"] == taskid;
      });
  ...
  loadOptions() {
    this.apiService.getJobOptions(null).then(res => {
      ...
      this.resolve_content =
        resolveTemplate != undefined ? resolveTemplate.content : "";
  ...
  initSummernote() {
    ...
    this.editor.summernote(
      "autoComplete.setDataSrc",
      function(keyword, callback) {
        callback(search(keyword));
      },
      "{"
    );
    this.editor.summernote("autoComplete.on", "insertSuggestion", function(
      suggestion
    ) {
  ...
  initResolveSummernote() {
    ...
    this.resolve_editor.summernote(
      "autoComplete.setDataSrc",
      function(keyword, callback) {
